...
export class Slide {
  constructor(slide, wrapper) {
    ...
    this.wraper = document.querySelector(wrapper);
    this.dist = {
      finalPosition: 0,
      startX: 0,
      movement: 0,
...
export default class SlideNav extends Slide {
  ...
  createControll() {
    ...
    this.slideArray.forEach((item, index) => {
      controll.innerHTML += `<li><a href="#slide${index + 1}">${
        index + 1
      }</a></li>`;
    });

    this.wraper.appendChild(controll);
    return controll;
  }
  ...
  eventControll(item, index) {
    ...
    this.wraper.addEventListener("changeEvent", this.activeControllItem);
  }
  ...
  addControll(customControll) {
    this.controll =
      document.querySelector(customControll) || this.createControll();
